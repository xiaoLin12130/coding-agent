    ...
    <div className="flex flex-col gap-3" data-testid="agents-panel">
        ...
        {running ? (
          <div data-testid="running-run" className="flex flex-col gap-1 text-[11px]">
            ...
            <p className="text-slate-500">
              {running.tool_calls} tool calls · rounds {running.round_count} ·{" "}
          ...
          <Empty>
            No run is running
            {live.runId ? " (last run " + live.runId + ")" : ""}.
          </Empty>
        )}

        {runId && (
          <p className="text-[10px] text-slate-600">console run id: {runId}</p>
        )}
      </section>
      ...
      <section className="flex flex-col gap-2 rounded-lg border border-edge bg-panelAlt p-3">
        ...
        <textarea
          ...
          placeholder="Task for the agent runtime"
      ...
      <section className="flex flex-col gap-2">
        <SectionTitle>Roles ({roles.length})</SectionTitle>
        {agents.status === "loading" && roles.length === 0 && <Spinner label="Loading roles…" />}
        {roles.length === 0 && agents.status !== "loading" ? (
          <Empty>No role information.</Empty>
        ) : (
            ...
            {roles.map((role) => (
              <li key={role.name} className="rounded-lg border border-edge bg-panelAlt px-2 py-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto text-[10px] text-slate-500">max {role.max_steps} steps</span>
                </div>
      ...
      <section className="flex flex-col gap-2">
        <SectionTitle>Run history ({runs.length})</SectionTitle>
        {runs.length === 0 ? (
          <Empty>No run has been recorded.</Empty>
        ) : (
